...
with torch.no_grad():
    ...
    while time.time() < start_loop + timeout :
        ...
        step_cost_list_stack.append(np.zeros((method_number,max_steps_episode,1)))
        
        flag_end_episode = False
        
        while not (Env_learning_lp.terminated or Env_learning_lp.truncated or Env_learning_nlp.terminated or Env_learning_nlp.truncated or Env_learning_lp_original.terminated or Env_learning_lp_original.truncated or Env_learning_nlp_original.terminated or Env_learning_nlp_original.truncated) and j < max_steps_episode:
            ...
            if (Env_learning_nlp.terminated or Env_learning_nlp.truncated) or (Env_learning_lp.terminated or Env_learning_lp.truncated) or (Env_learning_nlp_original.terminated or Env_learning_nlp_original.truncated) or (Env_learning_lp_original.terminated or Env_learning_lp_original.truncated) or j >= max_steps_episode:
                flag_end_episode = True
