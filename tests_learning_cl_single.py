...
import numpy as np
from rail_rl_env import RailNet, d_pre, rho_whole, un, ul, uy, ua, ud, ur, depot, mdl_feasible, action_dict, gurobi_lp_presolve
import time
...
with torch.no_grad():
    ...
    while time.time() < start_loop + timeout :
        ...
        while not (Env_learning_lp.terminated or Env_learning_lp.truncated) and j < 30:
            ...
            if (Env_learning_lp.terminated or Env_learning_lp.truncated) or j >= 30:
                flag_end_episode = True
